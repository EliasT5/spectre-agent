...
export function BlobScene() {
  ...
  // Adaptive resolution: PerformanceMonitor steps dpr down when the framerate
  // sags (e.g. the iGPU thermally throttles) and back up when it recovers, so a
  // heavy scene degrades gracefully instead of spiralling into ever-worse lag.
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 0, cursor: isDragging ? "grabbing" : "grab" }}>
      ...
      >
        <Canvas
          ...
          dpr={dpr}
          style={{ position: "absolute", inset: 0 }}
          // antialias is OFF: the EffectComposer (multisampling=0) renders the
          // scene into its own non-multisampled targets, so canvas-level MSAA
          // never reaches the screen — enabling it just burns GPU for nothing.
          gl={{ failIfMajorPerformanceCaveat: false, powerPreference: "high-performance", antialias: false }}
          onCreated={({ gl }) => {
