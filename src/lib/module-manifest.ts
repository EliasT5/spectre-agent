...
export interface ModuleManifestV2 {
  ...
  uiMode: ModuleUiMode;
  /**
   * UI descriptor — opaque to the shell. Data mode: a UI Schema v2 in
   * `ui.schema` (shared across devices) and/or device-specific `ui.mobile` /
   * `ui.desktop`. The host (/m/[id]) picks the device variant first, then the
   * shared `ui.schema`, then the other variant — so a module that ships only
   * ONE variant has it used on both mobile and desktop automatically. Code
   * mode: `ui.code` (entry/css/integrity).
   */
  ui?: unknown;
